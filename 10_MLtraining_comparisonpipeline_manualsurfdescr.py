...
Assumes a tidy dataframe with one row per (drug, surfactant) pair:
    drug_name | surfactant_name | <drug descriptors...> | <surfactant descriptors...> | solubility

This script supports two workflows:
1) Without feature reduction
2) With near-zero-variance feature reduction (both drug and surfactants descriptors filtered inside each CV training fold to avoid leakage)
...
from sklearn.model_selection import RepeatedKFold, LeaveOneGroupOut, GroupKFold, ShuffleSplit, GroupShuffleSplit, GridSearchCV, cross_validate # for cross-validation and hyperparameter tuning
...
# 0. Configuration
# ---------------------------------------------------------------------------
DATA_FILE = "10_drugsurf_moldescr_withnames_zerovariationfiltered_surfsMANUAL.csv" # name of file with rdkit descriptors for drugs, manual for surfs and solpow values
TARGET_COL = "solpow"  # column name for the target variable (solubilisation power, umol/g)
...
# If RUN_BOTH_MODES is True, the script runs both no-reduction and reduction modes.
# If RUN_BOTH_MODES is False, only USE_FEATURE_REDUCTION is used.
# If both are false, the script runs only the no-feature-reduction workflow.
# If both are true, the script runs both workflows and compares results.
RUN_BOTH_MODES = True
USE_FEATURE_REDUCTION = True

# Run all validation modes in one pass and compare them:
# 1) Standard no-holdout flow
# 2) Single holdout flow
# 3) Nested holdout flow - each repeat creates a never-seen holdout test split, tunes only on the
...
# Nested holdout mode.
# "random": random row-level holdout splits
# "grouped": drug-group holdouts (unseen-drug holdout each repeat)
NESTED_HOLDOUT_MODE = "grouped"
NESTED_HOLDOUT_TEST_SIZE = 0.2

# Single holdout mode.
# "random": random row-level holdout split
# "grouped": drug-group holdout split (unseen-drug strict)
SINGLE_HOLDOUT_MODE = "grouped"
SINGLE_HOLDOUT_TEST_SIZE = 0.2
...
# N.B.With just 5 surfactants, variance/correlation thresholds are unreliable — a small, chemically chosen set (e.g. HLB, CMC,
# MW) might be preferred over automated filtering. This will be done separately in a separate script 
# (10_MLtraining_comparisonpipeline_manualsurfdescr.py) to compare automated vs. manual surfactant descriptor selection.
...
# Outer split is grouped means train/test split is done by drug group, so the test set contains drugs not seen in that 
# outer training set.
# Inner loop can use GroupKFold means tuning folds are also split by drug groups, not random rows.
# keeps tuning strict: each inner validation fold also uses unseen-drug groups, so model selection is aligned with final evaluation goal.
# As outer mode is grouped, the inner loop can use GroupKFold on drug groups.
NESTED_GROUPED_INNER_USE_GROUPS = True
...
# CSV export settings.
RESULTS_OUTPUT_DIR = "10_ml_results"
...
y = df[TARGET_COL].to_numpy() # extracts target column (solpow) into a NumPy array for model training
...
# code to export a matplotlib figure to PNG, with timestamped filename and mode/figure tag for clarity.
def export_result_figure(fig_to_save: plt.Figure, mode_name: str, figure_tag: str) -> None:
    """Export one matplotlib figure to PNG."""

    output_dir = Path(RESULTS_OUTPUT_DIR)
    output_dir.mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    file_name = f"{timestamp}__{_slugify(mode_name)}__{figure_tag}.png"
    output_path = output_dir / file_name
    fig_to_save.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig_to_save)
    print(f"Saved: {output_path}")

# code to generate a parity plot comparing actual vs predicted values
def export_parity_plot(
    ...
    """Create and export parity scatter plot from observed/predicted test values."""
    if predictions_df.empty:
        print(f"Parity plot skipped ({figure_tag}): no prediction rows available.")
        return

    observed = predictions_df[observed_col].to_numpy(dtype=float)
    predicted = predictions_df[predicted_col].to_numpy(dtype=float)

    min_val = float(np.min(np.concatenate([observed, predicted])))
    max_val = float(np.max(np.concatenate([observed, predicted])))
    ...
    has_drug = "Drug" in predictions_df.columns
    has_surf = "Surfactant" in predictions_df.columns

    if has_drug and has_surf:
        # Pastel colors for surfactants.
        surf_categories = pd.Series(predictions_df["Surfactant"].astype(str)).fillna("NA").unique().tolist()
        pastel_colors = list(plt.cm.Pastel1.colors) + list(plt.cm.Pastel2.colors) + list(plt.cm.Set3.colors)
        surf_color_map = {
            surf_name: pastel_colors[idx % len(pastel_colors)]
            for idx, surf_name in enumerate(sorted(surf_categories))
        }

        # Distinct marker shapes for drugs.
        marker_pool = ["o", "s", "^", "v", "D", "P", "X", "<", ">", "h", "*", "p", "8"]
        drug_categories = pd.Series(predictions_df["Drug"].astype(str)).fillna("NA").unique().tolist()
        drug_marker_map = {
            drug_name: marker_pool[idx % len(marker_pool)]
            for idx, drug_name in enumerate(sorted(drug_categories))
        }

        plot_df = predictions_df.copy()
        plot_df["Drug"] = plot_df["Drug"].astype(str).fillna("NA")
        plot_df["Surfactant"] = plot_df["Surfactant"].astype(str).fillna("NA")

        # Draw one subset per (drug, surfactant) so both encodings are visible.
        for (drug_name, surf_name), block in plot_df.groupby(["Drug", "Surfactant"], dropna=False):
            ax.scatter(
                block[observed_col].to_numpy(dtype=float),
                block[predicted_col].to_numpy(dtype=float),
                s=44,
                alpha=0.88,
                marker=drug_marker_map[drug_name],
                color=surf_color_map[surf_name],
                edgecolors="#666666",
                linewidths=0.45,
            )

        # Separate legends keep mapping readable and avoid per-point legend clutter.
        surf_handles = [
            Line2D(
                [0],
                [0],
                marker="o",
                linestyle="",
                markerfacecolor=surf_color_map[surf_name],
                markeredgecolor="#666666",
                markeredgewidth=0.45,
                markersize=7,
                label=surf_name,
            )
            for surf_name in sorted(surf_color_map.keys())
        ]
        surf_legend = ax.legend(
            handles=surf_handles,
            title="Surfactant (color)",
            loc="upper left",
            fontsize=8,
            title_fontsize=8,
            framealpha=0.9,
        )
        ax.add_artist(surf_legend)
        ...
        drug_handles = [
            ...
            for drug_name in sorted(drug_marker_map.keys())
        ]
        ax.legend(
        ...
        # Fallback if identity columns are missing.
        ax.scatter(
            observed,
            predicted,
            s=36,
    ...
    low = min_val - pad
    high = max_val + pad
    ax.plot([low, high], [low, high], linestyle="--", linewidth=1.4, color="black", label="Ideal parity")
    ax.set_xlim(low, high)
    ax.set_ylim(low, high)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("Observed solpow")
    ax.set_ylabel("Predicted solpow")
    ax.set_title(title)
    ax.grid(alpha=0.25)

    stats_text = f"n = {len(predictions_df)}\nRMSE = {rmse:.4f}\nMAE = {mae:.4f}\nR2 = {r2:.4f}"
    ax.text(
        0.98,
        0.02,
        stats_text,
    ...
    export_result_figure(fig, mode_name, figure_tag)
...
class DrugSurfFeatureSelector(BaseEstimator, TransformerMixin): # class = custom scikit-learn transformer for leakage-safe 
    ...
    - If use_feature_reduction=False: keep all descriptor columns.
    - If use_feature_reduction=True:
    ...
    def __init__(
        ...
        use_feature_reduction: bool, # master switch for on/off filtering.
        ...
        self.use_feature_reduction = use_feature_reduction # save feature reduction on/off switch (toggle)
    ...
    def _filter_by_variance_and_correlation( 
        ...
        if target_column is not None and target_column in block_df.columns:
            target_corr = block_df[retained].corrwith(block_df[target_column]).abs()
    ...
    def fit(self, X, y=None):
        ...
        # If reduction is disabled, keep every descriptor exactly as provided.
        if not self.use_feature_reduction:
            self.selected_feature_names_ = list(self.descriptor_columns)
            return self
        ...
        # For surfactants, keep all provided descriptors for variance filtering
        # and only apply correlation pruning. This preserves manually chosen
        # surfactant descriptors while still reducing strong redundancy.
        retained_surf = list(self.surfactant_columns)
        if len(retained_surf) > 1:
            corr = unique_surf_df[retained_surf].corr().abs()
            if surf_target_column is not None and surf_target_column in unique_surf_df.columns:
                target_corr = unique_surf_df[retained_surf].corrwith(unique_surf_df[surf_target_column]).abs()
                order_lookup = {col: idx for idx, col in enumerate(retained_surf)}
                ordered = sorted(
                    retained_surf,
                    key=lambda col: (-target_corr.get(col, 0.0), order_lookup[col]),
                )
                kept = []
                for col in ordered:
                    if not any(corr.loc[col, kept_col] > self.surf_corr_threshold for kept_col in kept):
                        kept.append(col)
                retained_surf = kept
            else:
                upper = corr.where(np.triu(np.ones(corr.shape), k=1).astype(bool))
                to_drop = [col for col in upper.columns if any(upper[col] > self.surf_corr_threshold)]
                retained_surf = [c for c in retained_surf if c not in to_drop]
    ...
    # 1.0 is a common default value, but it can be tuned based on cross-validation or other methods.
        ...
        "Lasso": {
            ...
            "param_grid": { # hyperparameter grid for tuning the Lasso model - testing different values and choosing the best one based on cross-validation performance.
                "model__alpha": [1e-4, 1e-3, 1e-2, 1e-1, 1], # alpha controls the strength of the L1 penalty. Smaller values 
    ...
    # GP asks: “what family of smooth functions could explain the data, and how uncertain am I at each point?”
        ...
        "SVR": {
            ...
            "param_grid": {
                ...
                "model__epsilon": [0.01, 0.05, 0.1, 0.2, 0.5], # epsilon defines a margin of tolerance where no penalty is given to errors. It controls the width of the epsilon-insensitive tube. A larger epsilon allows more deviation from the true values without penalty, which can make the model more robust to noise.
    ...
    # at the end (leaf), prediction is usually the average target value of training points in that leaf
        ...
        "GradientBoosting": {
            ...
            "param_grid": {
                ...
                "model__learning_rate": [0.01, 0.05, 0.1], # learning rate shrinks the contribution of each tree. Lower values require more trees but can improve generalization.
...
# XGBoost added as David suggested it often performs well on tabular data.
    ...
    if HAS_XGBOOST:
        specs["XGBoost"] = {
            ...
            "param_grid": {
                "model__n_estimators": [100, 300], # number of trees to fit. More trees can improve performance but increase training time and risk of overfitting.
                "model__learning_rate": [0.03, 0.1], # learning rate shrinks the contribution of each tree. Lower values require more trees but can improve generalization.
                # controls how much each individual tree is allowed to contribute to the overall prediction during boosting
                "model__max_depth": [2, 4], # maximum depth of each tree. Controls model complexity. Deeper trees can capture more complex patterns but may overfit.
                "model__min_child_weight": [1, 3], # minimum sum of instance weight (hessian) needed in a child. Controls model complexity. Higher values prevent the model from learning overly specific patterns, reducing overfitting.
                "model__subsample": [0.8, 1.0], # fraction of samples used for fitting each tree. Lower values can improve generalization but may increase bias.
                "model__colsample_bytree": [0.7, 1.0], # fraction of features used for fitting each tree. Lower values can improve generalization but may increase bias.
            },
        }
...
# 4. Pipeline builder
# ---------------------------------------------------------------------------
def make_pipeline(estimator, needs_scaling: bool, use_feature_reduction: bool) -> Pipeline:
    ...
    feature_selector = DrugSurfFeatureSelector(
        ...
        use_feature_reduction=use_feature_reduction,
...
# staged XGBoost tuning strategy: Coarse compact grid first. Then a focused second grid around the best region.
# This usually gives better results than one huge brute-force search.
def _build_xgboost_refined_grid(best_params: dict) -> dict[str, list]:
    """Build a focused second-stage XGBoost grid around coarse best settings.

    To keep runtime practical, refinement expands four key hyperparameters and
    keeps subsample/colsample fixed at their coarse-best values.
    """
    # Extract values from pipeline-style keys.
    n_estimators_best = int(best_params.get("model__n_estimators", 200))
    learning_rate_best = float(best_params.get("model__learning_rate", 0.1))
    max_depth_best = int(best_params.get("model__max_depth", 3))
    min_child_weight_best = int(best_params.get("model__min_child_weight", 1))
    subsample_best = float(best_params.get("model__subsample", 1.0))
    colsample_best = float(best_params.get("model__colsample_bytree", 1.0))

    refined_grid = {
        "model__n_estimators": sorted(
            {
                max(50, n_estimators_best - 100),
                n_estimators_best,
        ...
        "model__learning_rate": sorted(
            {
                round(max(0.01, learning_rate_best * 0.6), 4),
                round(learning_rate_best, 4),
                round(min(0.3, learning_rate_best * 1.4), 4),
            }
        ),
        "model__max_depth": sorted(
            {
                max(2, max_depth_best - 1),
                max_depth_best,
                min(8, max_depth_best + 1),
            }
        ),
        "model__min_child_weight": sorted(
            {
                max(1, min_child_weight_best - 1),
                min_child_weight_best,
                min_child_weight_best + 1,
            }
        ),
        # Keep these fixed at coarse-best to avoid an unnecessary combinatorial blow-up.
        "model__subsample": [round(subsample_best, 4)],
        "model__colsample_bytree": [round(colsample_best, 4)],
    }
    return refined_grid
...
def tune_models(
    ...
    """Tune each model with repeated k-fold and return tuned estimators plus summary."""
    # RepeatedKFold is used here to make the tuning more stable than a single split. Each model will be evaluated across 
    # many train/validation splits, and the average score is used to decide which hyperparameters work best.
    tune_cv = RepeatedKFold(
    ...
    for model_name, spec in specs.items():
        ...
        pipe = make_pipeline(spec["estimator"], spec["needs_scaling"], use_feature_reduction)
        ...
        if model_name == "XGBoost":
            # Stage 1 (coarse) has finished above. Build a focused stage-2 grid around the coarse best params.
            coarse_best_params = search.best_params_
            refined_grid = _build_xgboost_refined_grid(coarse_best_params)

            refined_pipe = make_pipeline(spec["estimator"], spec["needs_scaling"], use_feature_reduction)
            ...
            refined_search.fit(X_data, y_vec)

            tuned_estimators[model_name] = refined_search.best_estimator_
            tuning_rows.append(
                {
                    "Model": model_name,
                    "BestCV_RMSE": -refined_search.best_score_,
                    "BestParams": refined_search.best_params_,
                    "XGB_CoarseBestCV_RMSE": -search.best_score_,
                    "XGB_CoarseBestParams": coarse_best_params,
                }
            )

            coarse_results_df = pd.DataFrame(search.cv_results_).copy()
            coarse_results_df.insert(0, "Model", model_name)
            coarse_results_df.insert(1, "TuneStage", "coarse")
            if "mean_test_score" in coarse_results_df.columns:
                coarse_results_df["mean_test_RMSE"] = -coarse_results_df["mean_test_score"]
            if "std_test_score" in coarse_results_df.columns:
                coarse_results_df["std_test_RMSE"] = coarse_results_df["std_test_score"]
            if "rank_test_score" in coarse_results_df.columns:
                coarse_results_df = coarse_results_df.sort_values("rank_test_score")

            refined_results_df = pd.DataFrame(refined_search.cv_results_).copy()
            refined_results_df.insert(0, "Model", model_name)
            refined_results_df.insert(1, "TuneStage", "refined")
            if "mean_test_score" in refined_results_df.columns:
                refined_results_df["mean_test_RMSE"] = -refined_results_df["mean_test_score"]
            if "std_test_score" in refined_results_df.columns:
                refined_results_df["std_test_RMSE"] = refined_results_df["std_test_score"]
            if "rank_test_score" in refined_results_df.columns:
                refined_results_df = refined_results_df.sort_values("rank_test_score")

            tuning_detail_tables.append(coarse_results_df)
            tuning_detail_tables.append(refined_results_df)
            continue
...
def report_feature_selection_stability(
    ...
    y_vec: np.ndarray,
    estimator: Pipeline,
    mode_name: str, # run mode (for example, with/without feature reduction)
) -> None:
    ...
    if drug_cols:
        drug_audit_rows = []
        for feature_name in sorted(drug_cols):
            retained_count = int(drug_feature_counter.get(feature_name, 0))
            filtered_count = int(total_folds - retained_count)
            retained_pct = (retained_count / total_folds) * 100 if total_folds > 0 else np.nan
            filtered_pct = (filtered_count / total_folds) * 100 if total_folds > 0 else np.nan

            if retained_count == total_folds:
                status = "Always retained"
            elif retained_count == 0:
                status = "Always filtered"
            else:
                status = "Sometimes filtered"

            drug_audit_rows.append(
                {
                    "Feature": feature_name,
                    "Retained_Folds": retained_count,
                    "Retained_Pct": retained_pct,
                    "Filtered_Folds": filtered_count,
    ...
    if surf_cols:
        surf_audit_rows = []
        for feature_name in sorted(surf_cols):
            retained_count = int(surf_feature_counter.get(feature_name, 0))
            filtered_count = int(total_folds - retained_count)
            retained_pct = (retained_count / total_folds) * 100 if total_folds > 0 else np.nan
            filtered_pct = (filtered_count / total_folds) * 100 if total_folds > 0 else np.nan
...
def run_nested_holdout(
    ...
    # This outer split creates repeated train/test partitions.
    # If grouped mode is used, the test split contains unseen drugs.
    if NESTED_HOLDOUT_MODE == "grouped":
        holdout_splitter = GroupShuffleSplit(
            n_splits=NESTED_HOLDOUT_N_REPEATS,
            test_size=NESTED_HOLDOUT_TEST_SIZE,
            random_state=RANDOM_STATE,
        )
        holdout_splits = list(holdout_splitter.split(X_data, y_vec, groups_vec))
    else:
        # Row-level random holdout if grouped splitting is not requested.
        holdout_splitter = ShuffleSplit(
            n_splits=NESTED_HOLDOUT_N_REPEATS,
            test_size=NESTED_HOLDOUT_TEST_SIZE,
            random_state=RANDOM_STATE,
        )
        holdout_splits = list(holdout_splitter.split(X_data, y_vec))
    ...
    for model_name, spec in specs.items():
        ...
        for repeat_idx, (train_idx, test_idx) in enumerate(holdout_splits, start=1):
            ...
            if NESTED_HOLDOUT_MODE == "grouped" and NESTED_GROUPED_INNER_USE_GROUPS:
                unique_train_groups = np.unique(groups_train)

                if len(unique_train_groups) >= 2: # need at least 2 unique groups to do GroupKFold
...
def run_single_holdout(
    ...
    """Run a single holdout split with inner tuning on train-only data."""
    # Create one fixed train/test split for this run.
    # In grouped mode, the split is done by drug group so the test set contains
    # unseen drugs. Otherwise the split is row-based.
    if SINGLE_HOLDOUT_MODE == "grouped":
        split_gen = GroupShuffleSplit(
            n_splits=1,
            test_size=SINGLE_HOLDOUT_TEST_SIZE,
            random_state=RANDOM_STATE,
        )
        train_idx, test_idx = next(split_gen.split(X_data, y_vec, groups_vec))
    else:
        ...
        train_idx, test_idx = next(split_gen.split(X_data, y_vec))
    ...
    # These lists accumulate one summary row per model plus a more detailed
    # record of the exact holdout split and chosen hyperparameters.
    summary_rows = []
    detail_rows = []
    prediction_rows = []
    inner_tuning_rows = []
    ...
    for model_name, spec in specs.items():
        # Inner CV is used only on the training portion so hyperparameter
        # tuning never sees the held-out test set.
        inner_cv = None
        inner_groups = None

        if SINGLE_HOLDOUT_MODE == "grouped" and NESTED_GROUPED_INNER_USE_GROUPS:
            unique_train_groups = np.unique(groups_train)

            if len(unique_train_groups) >= 2:
                # If there are enough unique training groups, use a grouped inner
                # split so the tuning step also respects the drug partitioning.
                n_inner_splits = min(NESTED_GROUPED_INNER_N_SPLITS, len(unique_train_groups))
                inner_cv = GroupKFold(n_splits=n_inner_splits)
                inner_groups = groups_train

        if inner_cv is None:
            # Fall back to repeated k-fold when grouped inner CV is not feasible.
            inner_cv = RepeatedKFold(
                n_splits=TUNE_N_SPLITS,
                n_repeats=TUNE_N_REPEATS,
                random_state=RANDOM_STATE,
        ...
        # Build the leakage-safe pipeline for this model and tune it only on
        # the training split.
        pipe = make_pipeline(spec["estimator"], spec["needs_scaling"], use_feature_reduction)
        ...
        # Archive split-level inner tuning results for this holdout run.
        model_cv_results_df = pd.DataFrame(search.cv_results_).copy()
        model_cv_results_df.insert(0, "Model", model_name)
        if "mean_test_score" in model_cv_results_df.columns:
            model_cv_results_df["mean_test_RMSE"] = -model_cv_results_df["mean_test_score"]
        if "std_test_score" in model_cv_results_df.columns:
            model_cv_results_df["std_test_RMSE"] = model_cv_results_df["std_test_score"]
        if "rank_test_score" in model_cv_results_df.columns:
            model_cv_results_df = model_cv_results_df.sort_values("rank_test_score")
        inner_tuning_rows.append(model_cv_results_df)

        # Evaluate the tuned model once on the untouched test split.
        best_model = search.best_estimator_
        y_pred = np.ravel(best_model.predict(X_test))

        # Collect point estimates for the single split.
        rmse = float(np.sqrt(mean_squared_error(y_test, y_pred)))
        mae = float(mean_absolute_error(y_test, y_pred))
        r2 = float(r2_score(y_test, y_pred))
        improvement_pct = float(((baseline_rmse - rmse) / baseline_rmse) * 100.0) if baseline_rmse > 0 else np.nan

        # The single holdout summary keeps the same column structure as the
        # repeated-CV tables, but the spread fields are not meaningful for one split.
        summary_rows.append(
            {
                "Model": model_name,
                "SingleHoldout_RMSE": rmse,
                "SingleHoldout_RMSE_std": "NaN", # set SDs to NaN because a single holdout split does not have a meaningful SD 
                "SingleHoldout_RMSE_min": rmse,
                "SingleHoldout_RMSE_max": rmse,
                "SingleHoldout_RMSE_range": 0.0,
                # repeat-based standard deviation, but this keeps the table structure consistent with the other summaries. 
                "SingleHoldout_MAE": mae,
                "SingleHoldout_MAE_std": "NaN",
                "SingleHoldout_MAE_min": mae,
                "SingleHoldout_MAE_max": mae,
                "SingleHoldout_MAE_range": 0.0,
                "SingleHoldout_R2": r2,
                "SingleHoldout_R2_std": "NaN",
                "SingleHoldout_R2_min": r2,
                "SingleHoldout_R2_max": r2,
                "SingleHoldout_R2_range": 0.0,
                "HeldOutDrugs": held_out_drugs_text,
                "Baseline_RMSE_TrainMean": baseline_rmse,
                "Baseline_RMSE_TrainMean_min": baseline_rmse,
                "Baseline_RMSE_TrainMean_max": baseline_rmse,
                "Baseline_RMSE_TrainMean_range": 0.0,
                "RMSE_ImprovementVsBaseline_Pct": improvement_pct,
                "RMSE_ImprovementVsBaseline_Pct_min": improvement_pct,
                "RMSE_ImprovementVsBaseline_Pct_max": improvement_pct,
                "RMSE_ImprovementVsBaseline_Pct_range": 0.0,
            }
        )

        # Store the raw holdout outcome and tuned parameters for auditability.
        detail_rows.append(
            {
                "Model": model_name,
                "TestRows": len(test_idx),
                "UniqueTestDrugs": len(np.unique(groups_test)),
                "HeldOutDrugs": held_out_drugs_text,
                "RMSE": rmse,
                "MAE": mae,
                "R2": r2,
    ...
    # Return a compact model-level summary plus the per-model details.
    summary_df = pd.DataFrame(summary_rows).sort_values("SingleHoldout_RMSE")
    details_df = pd.DataFrame(detail_rows).sort_values("RMSE")
    predictions_df = pd.DataFrame(prediction_rows).sort_values(["Model", "RowIndex"])
    inner_tuning_df = pd.concat(inner_tuning_rows, ignore_index=True)
    return summary_df, details_df, predictions_df, inner_tuning_df

print("Single holdout function defined.")
...
def run_standard_flow(
    ...
    print("\nTuning models...")
    tuned_estimators, tuning_df, tuning_details_df = tune_models(X_data, y_vec, specs, use_feature_reduction)
...
# ---------------------------------------------------------------------------
# 6. Run one mode (with or without feature reduction)
# ---------------------------------------------------------------------------
def run_mode(mode_name: str, use_feature_reduction: bool) -> None:
    # This wrapper runs the complete comparison workflow for one descriptor
    # setting: without feature reduction or with feature reduction.
    run_log_rows = []
    ...
    if use_feature_reduction:
        ...
        print("Feature reduction is OFF: all descriptor columns are used.")
        run_log_rows.append({"Stage": "Feature reduction", "Detail": "Setting", "Value": "OFF"})
    ...
    print("This run will execute and compare: no-holdout, single holdout, and nested holdout.")
    ...
    (
        ...
        tuning_df,
        tuning_details_df,
    ) = run_standard_flow(X_df, y, groups, specs, use_feature_reduction, mode_name)
    ...
    best_standard_model = repeated_df.iloc[0]["Model"]
    best_repeated_oof_agg = repeated_oof_agg_df[repeated_oof_agg_df["Model"] == best_standard_model]
    export_parity_plot(
        predictions_df=best_repeated_oof_agg,
        mode_name=mode_name,
        figure_tag="standard_repeated_oof_parity_plot",
        title=f"{mode_name} | {best_standard_model} | Repeated-CV OOF parity",
        observed_col="Observed",
        predicted_col="Predicted_mean",
    )

    best_logo_model = logo_df.iloc[0]["Model"]
    best_logo_oof = logo_oof_df[logo_oof_df["Model"] == best_logo_model]
    export_parity_plot(
        predictions_df=best_logo_oof,
        mode_name=mode_name,
        figure_tag="standard_logo_oof_parity_plot",
        title=f"{mode_name} | {best_logo_model} | LOGO OOF parity",
    )
    run_log_rows.append(
        {
            "Stage": "Standard flow",
            "Detail": "Best repeated-CV model",
            "Value": repeated_df.iloc[0]["Model"],
        }
    )
    run_log_rows.append(
        {
            "Stage": "Standard flow",
            "Detail": "Best repeated-CV RMSE",
            "Value": float(repeated_df.iloc[0]["RMSE"]),
        }
    )

    # Run the single holdout workflow, which uses one fixed train/test split and
    # tunes only on the training portion.
    print("\nSingle holdout mode:")
    print(f"Mode: {SINGLE_HOLDOUT_MODE}, Holdout fraction: {SINGLE_HOLDOUT_TEST_SIZE}")
    run_log_rows.append(
        {
            "Stage": "Single holdout",
            "Detail": "Split mode / holdout fraction",
            "Value": f"{SINGLE_HOLDOUT_MODE}, {SINGLE_HOLDOUT_TEST_SIZE}",
        }
    )
    single_summary_df, single_details_df, single_predictions_df, single_inner_tuning_df = run_single_holdout(X_df, y, groups, specs, use_feature_reduction)
    single_summary_df = add_error_percentage_columns(single_summary_df, y)
    single_details_df = add_error_percentage_columns(single_details_df, y)
    single_inner_tuning_df = add_error_percentage_columns(single_inner_tuning_df, y)
    print("\nSingle holdout results:")
    print(single_summary_df.round(4).to_string(index=False))
    print("\nSingle holdout details:")
    print(single_details_df.round(4).to_string(index=False))
    export_result_table(single_summary_df, mode_name, "single_holdout_summary")
    export_result_table(single_details_df, mode_name, "single_holdout_details")
    export_result_table(single_predictions_df, mode_name, "single_holdout_predictions")
    export_result_table(single_inner_tuning_df, mode_name, "single_holdout_inner_tuning_details")
    single_inner_tuning_top_df = (
        single_inner_tuning_df
        .sort_values(["Model", "rank_test_score", "mean_test_RMSE"])
        .groupby("Model", group_keys=False)
        .head(TOP_N_TUNING_ROWS_TO_EXPORT)
    )
    export_result_table(single_inner_tuning_top_df, mode_name, "single_holdout_inner_tuning_top_rows")
    best_single_model = single_summary_df.iloc[0]["Model"]
    best_single_predictions = single_predictions_df[single_predictions_df["Model"] == best_single_model]
    export_parity_plot(
        predictions_df=best_single_predictions,
        mode_name=mode_name,
        figure_tag="single_holdout_parity_plot",
        title=f"{mode_name} | {best_single_model} | Single holdout parity",
    )
    run_log_rows.append(
        {
            "Stage": "Single holdout",
            "Detail": "Best summary model",
            "Value": single_summary_df.iloc[0]["Model"],
        }
    )
    ...
    print("\nNested holdout mode:")
    print(f"Mode: {NESTED_HOLDOUT_MODE}, Repeats: {NESTED_HOLDOUT_N_REPEATS}, Holdout fraction: {NESTED_HOLDOUT_TEST_SIZE}")
    run_log_rows.append(
        {
            "Stage": "Nested holdout",
            "Detail": "Split mode / repeats / holdout fraction",
            "Value": f"{NESTED_HOLDOUT_MODE}, {NESTED_HOLDOUT_N_REPEATS}, {NESTED_HOLDOUT_TEST_SIZE}",
        }
    )
    nested_holdout_df, nested_holdout_repeats_df, nested_holdout_predictions_df, nested_inner_tuning_df = run_nested_holdout(X_df, y, groups, specs, use_feature_reduction)
    ...
    export_parity_plot(
        predictions_df=best_nested_predictions,
        mode_name=mode_name,
        figure_tag="nested_holdout_parity_plot",
        title=f"{mode_name} | {best_nested_model} | Nested holdout parity",
    )
    run_log_rows.append(
        {
            "Stage": "Nested holdout",
    ...
    comparison_df = (
        ...
        .merge(single_summary_df, on="Model", how="left")
        .merge(nested_holdout_df, on="Model", how="left")
    )
    comparison_df["SingleHoldout_minus_Standard_RMSE"] = (
        comparison_df["SingleHoldout_RMSE"] - comparison_df["Standard_RMSE"]
    )
...
if __name__ == "__main__":
    ...
    try:
        # Entry point: if both modes are enabled, run the workflow twice so we can
        # compare models with and without feature reduction.
        if RUN_BOTH_MODES:
            run_mode("Without Feature Reduction", use_feature_reduction=False)
            run_mode("With Feature Reduction", use_feature_reduction=True)
        else:
            # Otherwise run only the single mode selected in USE_FEATURE_REDUCTION.
            mode_label = "With Feature Reduction" if USE_FEATURE_REDUCTION else "Without Feature Reduction"
            run_mode(mode_label, use_feature_reduction=USE_FEATURE_REDUCTION)
    finally:

...
Assumes a tidy dataframe with one row per (drug, surfactant) pair:
    drug_name | surfactant_name | <drug descriptors...> | <surfactant descriptors...> | log_solubility

This script supports two workflows:
1) Without feature reduction
2) With feature reduction (both drug and surfactants descriptors filtered inside each CV training fold to avoid leakage)
...
# 0. Configuration
# ---------------------------------------------------------------------------
DATA_FILE = "07_drugsurf_moldescr_withnames_zerovariationfiltered.csv" # name of file with descriptors and solpow values
...
# (09_MLtraining_comparisonpipeline_manualsurfdescr.py) to compare automated vs. manual surfactant descriptor selection.
...
# CSV export settings.
RESULTS_OUTPUT_DIR = "07_ml_results"

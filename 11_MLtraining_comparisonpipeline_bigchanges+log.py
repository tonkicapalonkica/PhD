...
Assumes a tidy dataframe with one row per (drug, surfactant) pair:
    drug_name | surfactant_name | <drug descriptors...> | <surfactant descriptors...> | log solpow
...
# 0. Configuration
# ---------------------------------------------------------------------------
DATA_FILE = "11_drugsurf_moldescr_withnames_zerovariationfiltered_logsolpow.csv" # name of file with rdkit descriptors for drugs, manual for surfs and solpow values
TARGET_COL = "log(solpow)"  # column name for the target variable (log solubilisation power, umol/g)
...
# CSV export settings.
RESULTS_OUTPUT_DIR = "11_ml_results"
...
class DrugSurfFeatureSelector(BaseEstimator, TransformerMixin): # class = custom scikit-learn transformer for leakage-safe 
    ...
    def _filter_by_variance_and_correlation( 
        ...
        if target_column is not None and target_column in block_df.columns:
            target_series = block_df[target_column].astype(float)
            target_std = float(target_series.std(ddof=0))

            if np.isfinite(target_std) and target_std > 0.0:
                with np.errstate(divide="ignore", invalid="ignore"):
                    target_corr = block_df[retained].corrwith(target_series).abs()
        ...
        for feature_name in block_columns:
            ...
            if target_column is not None and target_column in block_df.columns:
                ...
                if (
                    ...
                    with np.errstate(divide="ignore", invalid="ignore"):
                        abs_target_corr = float(abs(feature_series.corr(target_series)))
            ...
            filtered_by_correlation = bool((not filtered_by_variance) and (feature_name not in retained_set))
    ...
    def fit(self, X, y=None):
        ...
        # Surfactant descriptors use the same variance-then-correlation filtering
        # as the drug block, but on the surfactant-specific training rows.
